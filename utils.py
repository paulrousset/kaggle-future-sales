...
def plot_quantiles(
    ...
    quantiles = df[var].quantile([0.01, 0.05, 0.1, 0.25, 0.5, 0.75, 0.9, 0.95, 0.99])

    sns.set_context("paper")
    sns.set_color_codes("muted")
    f, ax = plt.subplots(figsize=(width, height))
    sns.barplot(
        x=quantiles.values, y=quantiles.index.astype(str), color="b", edgecolor="w"
    )
...
def plot_barplots(
    ...
    """Plotting two barplots (volumes & sales)."""
...
def plot_horizontal_barplots(
    ...
    """Plotting two horizontal bar plots (volumes & sales)."""
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(width, height))
    if not share_axis:
        fig.subplots_adjust(wspace=0.6)
    # Volume Dimension
    volume.plot.barh(ax=ax1)
    ax1.set_title(title1)
    ax1.legend().set_visible(False)
    ax1.set(xlabel=None)
    # Sales Dimension
    if share_axis:
        sales.plot.barh(ax=ax2, sharex=ax1)
    else:
        sales.plot.barh(ax=ax2)
    ax2.set_title(title2)
    ax2.legend().set_visible(False)
    ax2.set(xlabel=None)


def get_quantiles_from_values(
    df: pd.DataFrame, variable: float, values: [], unit: str
) -> None:
...
shops_english = {
    "!Якутск Орджоникидзе, 56 фран": "Yakutsk - Ordzhonikidze",
    '!Якутск ТЦ "Центральный" фран': "Yakutsk - Central Mall",
    'Адыгея ТЦ "Мега"': "Adygea - Mega Mall",
    'Балашиха ТРК "Октябрь-Киномир"': "Balashikha - October-Kinomir",
    ...
    "Выездная Торговля": "Outbound - Outbound Trade",
    "Жуковский ул. Чкалова 39м?": "Moscow - Jukovski Airport",
    "Жуковский ул. Чкалова 39м²": "Moscow - Jukovski Airport",
    ...
    'Коломна ТЦ "Рио"': "Moscow - Kolomna Rio",
    ...
    'Химки ТЦ "Мега"': "Moscow - Mega Khimki"
}
